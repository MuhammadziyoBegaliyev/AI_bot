...
async def list_all_users(db: AsyncSession) -> list[User]:
    """
    Barcha foydalanuvchilarni qaytaradi (User obyektlari).
    Broadcast va Excel eksportda to‘g‘ridan-to‘g‘ri atributlar ishlatiladi.
    """
...
async def list_drugs(db: AsyncSession, limit: int = 50):
    res = await db.execute(select(Drug).order_by(Drug.name).limit(limit))
    return res.scalars().all()
...
async def find_drug_by_name(db: AsyncSession, name: str):
    res = await db.execute(select(Drug).where(Drug.name.ilike(f"%{name}%")))
    return res.scalars().all()
...
async def list_pharmacies(db: AsyncSession, limit: int = 100):
    res = await db.execute(select(Pharmacy).order_by(Pharmacy.title).limit(limit))
    return res.scalars().all()
...
async def all_pharmacies(db: AsyncSession):
    res = await db.execute(select(Pharmacy))
    return res.scalars().all()
...
async def list_searches(db: AsyncSession, limit: int = 1000):
    """
    Excel eksport uchun barcha (yoki limit) qidiruvlar.
    """
    res = await db.execute(
        select(SearchLog).order_by(SearchLog.created_at.desc()).limit(limit)
    )
    return res.scalars().all()
...
async def set_feedback_rating(db: AsyncSession, fb_id: int, rating: int):
    res = await db.execute(select(Feedback).where(Feedback.id == fb_id))
    fb = res.scalar_one()
    fb.rating = rating
    await db.commit()
    return fb
...
async def list_feedbacks(db: AsyncSession, limit: int = 200):
    res = await db.execute(select(Feedback).order_by(Feedback.created_at.desc()).limit(limit))
    return res.scalars().all()

# =====================================
# LOCATIONS (foydalanuvchi bo‘limi)
# =====================================
...
async def list_locations(db: AsyncSession, limit: int = 4):
    res = await db.execute(select(Location).limit(limit))
    return res.scalars().all()
